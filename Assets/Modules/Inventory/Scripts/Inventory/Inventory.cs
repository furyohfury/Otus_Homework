...
namespace Lessons.Meta.Lesson_Inventory
{
	...
	public class Inventory
	{
		...
		{
			if (TryFindItem(otherItem, out var item) == false)
			{
				Debug.LogWarning($"Couldn't find {otherItem.Name} in inventory to consume");
				return;
			}
			...
			{
				Debug.Log($"{otherItem.Name} is not consumable");
			}
		}
